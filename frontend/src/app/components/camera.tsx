...
export default function Camera({
  ...
  const capture = useCallback(
    async (pointDetails: PointDetails) => {
      console.log("ok");
      if (location) {
        ...
        if (imageSrc) {
          ...
          await captur(uint8Array, pointDetails);
        }
      } else {
  ...
  async function captur(uint8Array: Uint8Array, pointDetails: PointDetails) {
    console.log(pointDetails);
    if (lat && lng && uint8Array && pointDetails) {
